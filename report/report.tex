...
\centering
\small
\caption{Investable universe size by rebalance year}\label{tab:n-inv}
...
\noindent{\small\textit{Note.} Number of European firms eligible to
receive a non-zero weight at the December-$Y$ rebalance, i.e.\ firms
passing all five cleaning rules of Section~\ref{sec:cleaning} (price
floor, returns-from-prices with delisting, minimum 36 monthly
observations in the 120-month estimation window, stale-price filter,
non-missing carbon and revenue) and having a non-missing year-end
\textsc{ri} and \textsc{ci} in year $Y$. The universe grows
monotonically from 469 firms in 2013 to a peak of 600 in 2021 as
Datastream's Scope\,1$+$Scope\,2 coverage extends to mid-cap names;
the slight decline after 2021 reflects delistings (notably EDF in
2023 and Russian-listed names suspended in 2022).\par}
...
\noindent{\small\textit{Note.} Risk-return summary of the two Part~I
portfolios. Weights $w_Y^p$ are re-optimised every December
$Y\in\{2013,\dots,2024\}$ using the 120 trailing monthly returns and
applied to the following calendar year with end-of-month buy-and-hold
drift. Arithmetic return is monthly mean $\times 12$, geometric
return is $(1+R)_\text{prod}^{12/T}-1$, volatility is monthly std
$\times \sqrt{12}$, and the Sharpe ratios use a zero risk-free rate
as in the project template. Minimum-variance halves geometric return
($-338$\,bp) and trims volatility by $\sim$200\,bp, but its Sharpe
ratio is materially below the value-weighted benchmark in both
arithmetic ($0.34$ vs.\ $0.52$) and geometric terms ($0.27$ vs.\
$0.45$), consistent with the under-performance of low-volatility
tilts in trending bull markets.\par}
...
\noindent{\small\textit{Note.} Growth of \$1 held over the 12-year
out-of-sample horizon, compounded as $W_t = \prod_{s\le t}(1+R_s)$
from the monthly return series of each portfolio produced by the
annual rebalancing loop described in Section~\ref{sec:partI}. The
two curves track each other until 2017, after which the
technology-led bull market lifts the cap-weighted benchmark above
the minimum-variance allocation; the 2022 drawdown hits both
portfolios similarly but $P^{vw}$ recovers faster, ending the period
at \$2.3 versus \$1.5 for $P^{mv}$.\par}
...
\begin{table}[H]
\centering
\caption{Top 10 WACI contributors of $P^{vw}$ in 2013}
\label{tab:top10-vw}
\small
\begin{tabular}{rlrrr}
\toprule
...
\noindent{\small\textit{Note.} The ten largest contributors to the
value-weighted benchmark's WACI in the first rebalance year. For
each firm $i$ in the 2013 universe we compute $w_i^{vw}\cdot CI_i$
and sort descending. Ten names deliver 60.4 \si{tCO_2/M\$} out of
$\text{WACI}(P^{vw})_{2013}=136.1$, i.e.\ $\sim$44\% of the
benchmark's carbon intensity; the active-weight decomposition in
Section~\ref{sec:active} shows that the carbon-constrained portfolios
concentrate their adjustments on this short list.\par}
...
\caption{Year-by-year carbon footprint and WACI, split by portfolio
family}
\label{fig:part2-cf-split}
\end{figure}

\noindent{\small\textit{Note.} Left panel shows the MV family, right
panel the VW family. CF and WACI are computed by applying year-$Y$
weights to $E_{i,Y}/\text{MV}_{i,Y}$ (for CF) and $CI_{i,Y}$ (for
WACI) per equations~\eqref{eq:waci}--\eqref{eq:cf}; the dashed yellow
line on the right panel is the NZ budget
$0.9^{(Y-2012)}\cdot CF(P^{vw})_{2013}$. The VW benchmark
decarbonises by $\sim$58\% over the sample (CF from 214 to 90
\si{tCO_2/M\$}) and both VW-constrained variants bind in every year.
The MV portfolio is structurally low-CF (below VW throughout); the
2014--2017 spike is the EVRAZ episode detailed in
Appendix~\ref{app:carbon}.\par}

Two findings on Figure~\ref{fig:part2-cf-split} are worth flagging.

\emph{First}, $CF(P^{vw})$ falls from 214 \si{tCO_2/M\$} in 2013 to
about 90 in 2024 --- a 58\% reduction without any portfolio
intervention. The decline is consistent with a combination of
European regulatory pressure (the EU ETS price moved from
$\sim$\EUR{5} to $\sim$\EUR{80}/\si{tCO_2} over the period),
coal-to-gas and nuclear substitution effects, and changes in the
value-weighted composition of the index toward technology and
healthcare. We do not attempt to attribute the decline to these
mechanisms quantitatively.

\emph{Second}, $CF(P^{mv})$ is consistently below $CF(P^{vw})$ ---
27 vs.\ 214 in 2013, and 64 vs.\ 90 in 2024. With no carbon objective
in the optimisation, this is consistent with the minimum-variance
optimiser favouring lower-volatility names, which in this sample
tend to be less commodity-exposed than the heavyweights of $P^{vw}$.
...
\begin{table}[H]
\centering
\caption{Four-way performance comparison: MV and VW with and without
a 50\% carbon cap}\label{tab:34}
\small
...
\noindent{\small\textit{Note.} Risk-return summary of the two
benchmark portfolios and their carbon-constrained counterparts. Same
backtest mechanics as Table~\ref{tab:part1-stats} (annual
rebalancing, buy-and-hold within year). The carbon-constrained
portfolios solve the QPs of equations~\eqref{eq:mv05}
and~\eqref{eq:vw05} with $CF(P)\le 0.5\cdot CF(\text{anchor})$,
recomputed every year. Horizontal pairs isolate the observed cost
of a 50\% carbon reduction inside each allocation philosophy. Adding
the cap to $P^{mv}$ is observed to be slightly accretive ($+21$\,bp
geometric, lower volatility) --- a result consistent with the
constrained portfolio moving away from concentrated positions in
high-carbon cyclicals, although we do not test that mechanism
formally. The VW cap shows a $-38$\,bp geometric ($-50$\,bp
arithmetic) drag for a 50\% carbon reduction.\par}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{part2_cumulative_returns.png}
\caption{Section 3.4 cumulative returns of the four portfolios}
\label{fig:part2-cum}
\end{figure}

\noindent{\small\textit{Note.} Growth of \$1 invested in each
portfolio between Jan 2014 and Dec 2025. The OOS monthly return
series of each portfolio is compounded as
$W_t=\prod_{s\le t}(1+R_s)$; the left panel pairs the
minimum-variance family, the right panel the value-weighted family.
The dashed $P^{mv}(0.5)$ ends slightly above $P^{mv}$ in the left
panel; the dashed $P^{vw}(0.5)$ ends $\sim$5\% below $P^{vw}$ in the
right panel after 12 years of compounding.\par}
...
\caption{Carbon trajectory of the value-weighted family with the
Net-Zero glide path}
\label{fig:part2-nz-cf}
\end{figure}

\noindent{\small\textit{Note.} Year-by-year CF (\si{tCO_2/M\$}) of
$P^{vw}$, $P^{vw}(0.5)$ and $P^{vw}(NZ)$ together with the NZ budget.
$CF_Y=\sum_i w_{i,Y}\,E_{i,Y}/\text{MV}_{i,Y}$ using the year-$Y$
weights; the NZ budget is
$0.9^{(Y-2012)}\cdot CF(P^{vw})_{2013}=0.9^{(Y-2012)}\cdot 214.2$.
Both constrained portfolios bind in every year. The NZ line tracks
$P^{vw}$ until 2017 (in our sample the index decarbonises faster
than 10\% pa over 2013--2017) and then detaches as the glide path
becomes the binding constraint.\par}
...
\begin{table}[H]
\centering
\caption{Net-Zero comparison of $P^{vw}$, $P^{vw}(0.5)$ and
$P^{vw}(NZ)$}\label{tab:42}
\small
\begin{tabular}{l S[table-format=2.2] S[table-format=2.2] S[table-format=2.2] S[table-format=1.2]}
\toprule
                       & {Arith.\ (\%)}  & {Geom.\ (\%)} & {Vol.\ (\%)} & {SR$_{\text{ar}}$} \\
...
\noindent{\small\textit{Note.} Risk-return summary of $P^{vw}$,
$P^{vw}(0.5)$ and $P^{vw}(NZ)$ over Jan 2014 -- Dec 2025. Same
...
static $0.5\cdot CF(P^{vw})_Y$. $P^{vw}(NZ)$ underperforms $P^{vw}$
by 32\,bp pa geometric --- less than the 38\,bp drag of the static
50\% cap --- while ending the horizon with a 72\% carbon reduction
vs.\ the benchmark.\par}
...
\begin{table}[H]
\centering
\caption{Robustness of $P^{vw}(0.5)$ to cleaning and covariance
choices}\label{tab:robustness}
\small
...
\noindent{\small\textit{Note.} Annualised return, volatility and
geometric Sharpe of $P^{vw}(0.5)$ under five deviations from
baseline. Each row changes one parameter (stale-price threshold,
minimum history, or covariance estimator) and re-runs the entire
2013--2024 backtest; the Ledoit--Wolf row uses the linear shrinkage
estimator of \citet{LedoitWolf2004}. The spread across scenarios is
4\,bp pa on geometric return and $<$2\,bp on volatility --- results
are insensitive to the numerical choices we tested.\par}
...
\begin{table}[H]
\centering
\caption{Active weights of $P^{vw}(0.5)$ vs.\ $P^{vw}$, 2013 and 2024}
...
\noindent{\small\textit{Note.} The five largest over- and
underweights of $P^{vw}(0.5)$ relative to $P^{vw}$ in percentage
points, in the first and last rebalance years.
$\Delta w_i = w_i^{vw,0.5} - w_i^{vw}$, with the constrained weights
solving~\eqref{eq:vw05} under $\bar c = 0.5\,CF(P^{vw})_Y$. The
underweights are concentrated on a handful of high-intensity
industrial and utility names; the overweights are diffuse across
low-intensity firms --- a pattern consistent with the right-skewed
carbon distribution.\par}
...
\begin{table}[H]
\centering
\caption{Country-level active tilt of $P^{vw}(0.5)$, averaged across
2013--2024}\label{tab:country-tilt}
\small
...
\noindent{\small\textit{Note.} Mean of
$\sum_{i \in c} (w_i^{vw,0.5} - w_i^{vw})$ by listing country $c$,
averaged across rebalance years (percentage points). The pattern is
consistent with a substitution of carbon-heavy German industrials
and Swiss/Danish cyclicals into UK-listed services; magnitudes stay
$\le 40$\,bp, consistent with the tracking-error penalty
of~\eqref{eq:vw05}.\par}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{No transaction costs and no turnover analysis.} Annual
        rebalancing at the index level masks realistic frictions
        (bid-ask, market impact, tax). The MV portfolio is most
        affected: with 10--22\% single-name weights and no
        no-trade buffer, year-on-year turnover is mechanically high.
        $P^{vw}(0.5)$ and $P^{vw}(NZ)$ inherit lower turnover from the
        cap-weighted anchor but still trade against the active-weight
        tilts of Section~\ref{sec:active} each year. We do not
        quantify either turnover or its associated costs; a real
        implementation should report both alongside performance.
...
\begin{table}[H]
\centering
\caption{Full year-by-year carbon metrics for the five portfolios}
...
\noindent{\small\textit{Note.} WACI and CF (\si{tCO_2/M\$}) of every
portfolio in every rebalance year, plus the NZ budget target for
reference. $\text{WACI}_Y=\sum_i w_{i,Y}\,CI_{i,Y}$ and
$\text{CF}_Y=\sum_i w_{i,Y}\,E_{i,Y}/\text{MV}_{i,Y}$, using the
weights from the year-$Y$ optimisation. The achieved CF of
$P^{vw}(0.5)$ matches its cap in every year to within
$0.1$ \si{tCO_2/M\$}, and the achieved CF of $P^{vw}(NZ)$ matches the
NZ budget exactly --- the empirical signature of a binding constraint
at a well-converged KKT point.\par}
...
\begin{table}[H]
\centering
\caption{Allocation diagnostics by rebalance year}
\label{tab:alloc}
\scriptsize
\setlength{\tabcolsep}{4pt}
...
\noindent{\small\textit{Note.} Universe size $|U_Y|$, largest
single-name weight in each portfolio (percent), and the binding
carbon-cap level (\si{tCO_2/M\$}). Recorded at the end of each year's
optimisation loop. The MV portfolios reach 10--22\% single-name
weights, while the VW-anchored constrained portfolios stay at
2.7--3.4\%, consistent with the tracking-error penalty pinning them
close to the benchmark's concentration. The cap columns show the
constraints tightening over time as the European index
self-decarbonises.\par}
...
\caption{MV-family carbon metrics on a stand-alone axis}
\label{fig:mv-family-cf}
\end{figure}

\noindent{\small\textit{Note.} WACI (right $y$) and CF (left $y$) of
$P^{mv}$ and $P^{mv}(0.5)$, 2013--2024, computed via
equations~\eqref{eq:waci}--\eqref{eq:cf}. Plotted on a dedicated
scale because the MV-family numbers are 5--10$\times$ smaller than
the VW family. Both curves are structurally low; the spikes on
$P^{mv}$ in 2014--2017 are driven by a single high-CI name (EVRAZ),
which $P^{mv}(0.5)$ tames mechanically via the annual recomputation
of the cap.\par}
...
\caption{Cumulative excess return of the carbon-constrained VW
portfolios relative to $P^{vw}$}
\label{fig:excess}
\end{figure}

\noindent{\small\textit{Note.} Cumulative log-ratio
$\ln(W^{p}_t / W^{vw}_t)$ for $p \in \{P^{vw}(0.5), P^{vw}(NZ)\}$,
Jan 2014 -- Dec 2025. Both return series are compounded
independently and the ratio is plotted on a log scale, so that a
$-1\%$ slope equates to $1\%$ pa underperformance. Both constrained
portfolios stay within $\pm 2\%$ of the benchmark until 2020, then
drift $\sim$4--5\% below over 2020--2022 (a period of strong recovery
in high-carbon energy and materials names) and partially recover
thereafter.\par}
