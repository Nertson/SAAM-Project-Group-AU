...
benchmark's WACI in the first rebalance year. \emph{How constructed:}
for each firm $i$ in the 2013 universe we compute
$w_i^{vw}\cdot CI_i$ and sort descending. \emph{How to read:} ten
names deliver $60.4$ \si{tCO_2/M\$} out of
$\text{WACI}(P^{vw})_{2013}=136.1$ ($\sim$44\% of the benchmark's
carbon intensity). The active-weight decomposition in
Section~\ref{sec:active} shows that the carbon-constrained portfolios
indeed concentrate their adjustments on this short list.}
\label{tab:top10-vw}
\small
\begin{tabular}{rlrrr}
\toprule
...
\caption{\textbf{Year-by-year carbon footprint (CF) and WACI, split
by portfolio family.} \emph{What:} MV family (left) and VW family
(right). \emph{How constructed:} year-$Y$ weights applied to
$E_{i,Y}/\text{MV}_{i,Y}$ (CF) and $CI_{i,Y}$ (WACI) per
equations~\eqref{eq:waci}--\eqref{eq:cf}; dashed yellow on the right
panel is the NZ budget $0.9^{(Y-2012)}\cdot CF(P^{vw})_{2013}$.
\emph{How to read:} the VW benchmark decarbonises by $\sim$58\% over
the sample (CF from 214 to 90 \si{tCO_2/M\$}); both VW-constrained
variants bind in every year. The MV portfolio is structurally low-CF
(below VW throughout); the 2014--2017 spike is the EVRAZ episode
detailed in Appendix~\ref{app:carbon}.}
...
year. \emph{How to read:} horizontal pairs isolate the observed cost of a
50\% carbon reduction inside each allocation philosophy. Adding the
cap to $P^{mv}$ is observed to be slightly accretive ($+21$\,bp
geometric, lower volatility), a result consistent with the
constrained portfolio moving away from concentrated positions in
high-carbon cyclicals; we do not test that mechanism formally. The VW
cap shows a $-38$\,bp geometric ($-50$\,bp arithmetic) drag for a
50\% carbon reduction.}\label{tab:34}
\small
...
family. \emph{How to read:} the dashed $P^{mv}(0.5)$ ends slightly
above $P^{mv}$ in the left panel; the dashed $P^{vw}(0.5)$ ends
$\sim$5\% below $P^{vw}$ in the right panel after 12 years of
compounding.}
\label{fig:part2-cum}
\end{figure}
...
the Net-Zero glide path.} \emph{What:} year-by-year CF
(\si{tCO_2/M\$}) of $P^{vw}$, $P^{vw}(0.5)$ and $P^{vw}(NZ)$ together
with the NZ budget. \emph{How constructed:}
...
\emph{How to read:} both constrained portfolios bind in every year.
The NZ line tracks $P^{vw}$ until 2017 (the index decarbonises faster
...
risk-return summary of $P^{vw}$, $P^{vw}(0.5)$ and $P^{vw}(NZ)$ over
Jan 2014 -- Dec 2025. \emph{How constructed:} same backtest as
Table~\ref{tab:34}; the NZ portfolio solves~\eqref{eq:vw05} every
year with a time-varying cap
$\bar c_Y = 0.9^{(Y-2012)}\cdot CF(P^{vw})_{2013}$ instead of the
static $0.5\cdot CF(P^{vw})_Y$. \emph{How to read:} $P^{vw}(NZ)$
underperforms $P^{vw}$ by 32\,bp pa geometric --- less than the
38\,bp drag of the static 50\% cap --- while ending the horizon with
a 72\% carbon reduction vs.\ the benchmark.}\label{tab:42}
\small
\begin{tabular}{l S[table-format=2.2] S[table-format=2.2] S[table-format=2.2] S[table-format=1.2]}
\toprule
                       & {Arith.\ (\%)}  & {Geom.\ (\%)} & {Vol.\ (\%)} & {SR$_{\text{ar}}$} \\
...
choices.} \emph{What:} annualised return, volatility and geometric
Sharpe of $P^{vw}(0.5)$ under five deviations from baseline.
\emph{How constructed:} each row changes one parameter
(stale-price threshold, minimum history, or covariance estimator) and
re-runs the entire 2013--2024 backtest; the Ledoit--Wolf row uses
the linear shrinkage estimator of \citet{LedoitWolf2004}.
\emph{How to read:} the spread across scenarios is 4\,bp pa on
geometric return and $<$2\,bp on volatility --- results are
insensitive to the numerical choices we tested.}\label{tab:robustness}
\small
...
and 2024.} \emph{What:} the five largest over- and underweights in
percentage points, in the first and last rebalance years.
\emph{How constructed:}
$\Delta w_i = w_i^{vw,0.5} - w_i^{vw}$, with the constrained weights
solving~\eqref{eq:vw05} under $\bar c = 0.5\,CF(P^{vw})_Y$.
\emph{How to read:} the underweights are concentrated on a handful
of high-intensity industrial and utility names; the overweights are
diffuse across low-intensity firms --- a pattern consistent with the
right-skewed carbon distribution.}
...
averaged across 2013--2024.} \emph{What:} mean of
$\sum_{i \in c} (w_i^{vw,0.5} - w_i^{vw})$ by listing country $c$
across rebalance years (pp). \emph{How constructed:} sum the
firm-level active weights within each country in each year, then
average. \emph{How to read:} the pattern is consistent with a
substitution of carbon-heavy German industrials and Swiss/Danish
cyclicals into UK-listed services; magnitudes stay $\le 40$\,bp,
consistent with the tracking-error penalty
of~\eqref{eq:vw05}.}\label{tab:country-tilt}
\small
...
reference. \emph{How constructed:}
$\text{WACI}_Y=\sum_i w_{i,Y}\,CI_{i,Y}$ and
$\text{CF}_Y=\sum_i w_{i,Y}\,E_{i,Y}/\text{MV}_{i,Y}$ using the
weights from the year-$Y$ optimisation. \emph{How to read:} the
achieved CF of $P^{vw}(0.5)$ matches its cap in every year to within
...
\emph{What:} universe size $|U_Y|$, largest single-name weight in
each portfolio, and the binding carbon-cap level. Weight columns are
in percent; cap columns in \si{tCO_2/M\$}.
\emph{How constructed:} recorded at the end of each year's
optimisation loop. \emph{How to read:} MV reaches 10--22\% single
names; the VW-anchored constrained portfolios stay at 2.7--3.4\%,
consistent with the tracking-error penalty pinning them close to the
benchmark's concentration. The cap columns show the constraints
tightening over time as the European index self-decarbonises.}
\label{tab:alloc}
\scriptsize
\setlength{\tabcolsep}{4pt}
...
\caption{\textbf{MV-family carbon metrics, stand-alone axis.}
\emph{What:} WACI (right $y$) and CF (left $y$) of $P^{mv}$ and
$P^{mv}(0.5)$, 2013--2024. \emph{How constructed:} formulas
(\ref{eq:waci})--(\ref{eq:cf}), plotted on a dedicated scale because
the MV-family numbers are 5--10$\times$ smaller than the VW family.
\emph{How to read:} both curves are structurally low; the spikes on
$P^{mv}$ in 2014--2017 are driven by a single high-CI name (EVRAZ);
$P^{mv}(0.5)$ tames them mechanically via the annual recomputation
of the cap.}
...
log-ratio $\ln(W^{p}_t / W^{vw}_t)$ for
$p \in \{P^{vw}(0.5), P^{vw}(NZ)\}$, Jan 2014 -- Dec 2025.
\emph{How constructed:} both return series are compounded
independently and the ratio is plotted on a log scale, so that a
$-1\%$ slope equates to $1\%$ pa underperformance. \emph{How to read:}
both constrained portfolios stay within $\pm 2\%$ of the benchmark
until 2020, then drift $\sim$4--5\% below over 2020--2022 (a period
of strong recovery in high-carbon energy and materials names), and
partially recover thereafter.}
\label{fig:excess}
\end{figure}
